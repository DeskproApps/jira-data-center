...
const SubtaskDropdownWithSearch: FC<DropdownWithSearchProps> = ({ helpers, id, placeholder, value, projectId, ...props }) => {
  const { client } = useDeskproAppClient();
  const [loading, setLoading] = useState<boolean>(false);
  const [isDirtySearch, setIsDirtySearch] = useState(false);
  const [searchQuery, setSearchQuery] = useState<string>("");
  const [parents, setParents] = useState<Record<string, IssueItem>>({});
  const [parentOptions, setParentOptions] = useState<DropdownValueType<IssueItem["key"]>[] | DropdownHeaderType[]>([]);
...
export { SubtaskDropdownWithSearch };
